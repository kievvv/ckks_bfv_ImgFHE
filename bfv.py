...
def restored_image():
    ...
    for i in range(1, 101):  # 读取图片
        ...
        values_red, values_green, values_blue = encrypt_image(image_path, context, ts.SCHEME_TYPE.BFV)
...
def encrypt_image(image_path, context, encryption_algorithm):
    ...
    # 加密每个通道，使用公钥
    encrypted_red = ts.bfv_vector(context, red_channel)
    encrypted_green = ts.bfv_vector(context, green_channel)
    encrypted_blue = ts.bfv_vector(context, blue_channel)

    return encrypted_red, encrypted_green, encrypted_blue

def calculate_cosine_similarity(vector1, vector2, bfv_secret_key):
    # 分子
    ...
    numerator = numerator.decrypt(secret_key = bfv_secret_key)
    denominator = denominator.decrypt(secret_key = bfv_secret_key)
...
def decrypt_all_image():
    ...
    # 创建保存目录
    save_directory = "./bfv_restored_images/"
    ...
    for i, row in enumerate(rows, start=1):
        ...
        # 保存还原后的图像为 JPG 文件
        restored_image.save(f"./bfv_restored_images/{i}_restored.jpg")
        print(f"解密还原第{i}张图片")
...
if __name__ == "__main__":
    ...
    restored_image()
    t_end_add = time()
    print("bfv同态加密用时: {} ms".format((t_end_add - t_start_add) * 1000))
    ...
    encrypted_query_red, encrypted_query_green, encrypted_query_blue = encrypt_image(query_image_path, context, ts.SCHEME_TYPE.BFV)
    ...
    for i, row in enumerate(rows, start=1):
        # 反序列化数据库中的密文向量
        serialized_red, serialized_green, serialized_blue = row
        encrypted_red = ts.bfv_vector_from(context, serialized_red)
        encrypted_green = ts.bfv_vector_from(context, serialized_green)
        encrypted_blue = ts.bfv_vector_from(context, serialized_blue)

        # 计算余弦相似度
        similarity_red = calculate_cosine_similarity(encrypted_query_red, encrypted_red,bfv_secret_key)
        similarity_green = calculate_cosine_similarity(encrypted_query_green, encrypted_green,bfv_secret_key)
        similarity_blue = calculate_cosine_similarity(encrypted_query_blue, encrypted_blue,bfv_secret_key)
